...
**状态**：**切片 1/2/3 全部完成并通过等价性验收（2026-09-16）；移植收官**
...
### 切片 3：`_negamax` + Star1 机会节点 → C++（**已完成，2026-09-16**）

- **交付形态是"混合边界"**：C++ 拥有 `_negamax`（TT + PVS + 路径重复 + 杀手/历史）
  与 `_evaluate_chance_flip`（Star1 + 解析期望）；**迭代加深的根循环留在 Python**，
  因为 `degraded` / `avoid` / `exact_root_scores` / `root_scores` / `_is_tactical`
  这些最易错的语义原地复用比移植更划算。
- **实测（depth=2，四档决策全一致）**：开局 **1542 → 35 ms（43.70×）**、
  中盘 29.77×、midgame2 31.99×、残局 4194 → 221 ms（18.98×）。
  开局从切片 2 的 415 ms 再降 **11.8×** —— 正是切片 2 结尾预判的收益点。
- **验收**：✅ `perf_baseline verify` → `EQUIVALENT`；子树直接比对逐位一致；
  全量 534 passed / 3 skipped；限时路径更深 4 / 一致 0 / 更浅 0。
- **踩坑（已固化）**：`encode_state_blob` 漏传 `winner` ⇒ 已终局子状态被当成未终局，
  一步扛旗被估成 350 分。症状只在切片 3 暴露（切片 2 的入口状态必非终局）。
  另：pybind11 要求基类先注册，否则 `HAS_CPP_CORE=False`、C++ 路径**静默全退回 Python**，
  等价性测试会假通过 —— 验证脚本已加 `assert HAS_CPP_CORE`。
...
5. ✅ **切片 3** —— 已交付（2026-09-16）。开局 1542 → 35 ms（43.70×）。
   实际约 430 行（不是预估的 1500~2000 行）：**混合边界**（根循环留 Python）
   把工作量砍掉了大半，同时把最易出错的部分留在原地。
6. ⬜ **是否把根循环（IDS）也搬进 C++？** 收益已很有限（根循环只占残局 221 ms 的
   一小部分，且切片 3 后开局已顶到 `max_depth` 上限）。风险高（`degraded` 语义），
   建议**不做**，除非将来出现新的瓶颈证据。
