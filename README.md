...
> - ✅ **（2026-09-17）数据集切分可审计化（根治跨版本评测泄漏）**：导出时把每个 split 的 `.sav` 文件名清单落盘进 `metadata.json`；新增冻结 test 机制（`--frozen-test datasets/canonical_test.json`，test 恒等于 canonical 清单、整批排除出 train/val）；新增 `scripts/audit_dataset_leakage.py` 做跨版本 3×3 交集矩阵与 canonical 守卫（含"零个可审计数据集 ≠ 无泄漏"的兜底）。已反推 `p1_v3` 切分清单（局数+plies 逐项吻合）并冻结 96 局 canonical test，导出 `datasets/p1_v4`（leak_free=True）。
> - ✅ **（2026-09-17）修复混合引擎的重复局面规避缺陷（P2 验收第 4 条）**：`junqi/ai.py` 的 `HybridAgent.choose_actions` 把 `avoid` 判定写成 `a in avoid`（`a` 是 `Action`，而 `avoid` 装的是 `position_key()` 的局面键）⇒ **恒不命中，从不规避重复局面**。修正为 `position_key(nxt) in avoid` 并限定走子（翻子会永久增加公开信息，其局面键不可能与历史键重合）。新增审计脚本 `scripts/audit_p2_game_quality.py`：重复判和率 **24/60 = 40% → 2/60 = 3.3%**（对照 `expert2` 6.7%），非法 0、送旗 1/5798，第 4 条由未通过转为**通过**。
> - ✅ **（2026-09-17）P2 验收第 2、3 条补测**：③ 门控用修复后的引擎复跑 —— 得分率 0.5225 → **0.5475**（Wilson [0.4783, 0.6149]，非劣成立），配对检验 p 0.0463 → **0.0002**，`repetition` 判和 75 → **6**。② 新增 `scripts/audit_p2_phase_stability.py` 做**决策级全量覆盖**压测（三阶段各 200 快照 × 2 引擎，检查异常/返回空/非法动作），六组合**零违规**；对局级三阶段审计 `endgame` ✅、`opening`/`midgame` ❌（两个既有行为探针：往复踱步 2.80×、龟缩拒翻 1.84× 于对照），并用「`git show HEAD:junqi/ai.py` 覆写对照副本」证明该劣势**非本次修复引入**（修复使三阶段全部改善、开局重复判和 75% → 8.3%）。
